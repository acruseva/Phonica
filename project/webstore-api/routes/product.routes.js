...
// GET products list
router.get('/:productId', verifyToken, function (req, res) {
    const db = req.app.locals.db;
...
router.put('/:productId', verifyToken, verifyRoleOrSelf(3, false), function (req, res) {
    const db = req.app.locals.db;
    const product = req.body;
    indicative.validate(product, {
...
router.delete('/:productId', verifyToken, verifyRoleOrSelf(3, false), function (req, res) {
    const db = req.app.locals.db;
    const params = req.params;
    indicative.validate(params, { productId: 'required|regex:^[0-9a-f]{24}$' })
        .then(() => {
